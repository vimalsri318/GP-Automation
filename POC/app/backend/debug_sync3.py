...
# The current cache lives in the backend folder
...
for s_pkl in so_listing_files:
    try:
        ...
        if match_rows < len(targets_total):
            ...
            alt1_match = targets_total[targets_total.isin(alt1_keys)].count()
            print(f"🏁 Alt Matching (No Decimal Strip): {alt1_match}")
            ...
            alt2_match = targets_total[targets_total.isin(alt2_keys)].count()
            print(f"🏁 Alt Matching (No Zero Strip): {alt2_match}")

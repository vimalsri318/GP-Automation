...
# Absolute paths confirmed via list_dir
...
def debug_step2_columns():
    ...
    for label, filename in files.items():
        ...
        try:
            # Sheet guess:
            # Revenue/Cost usually have data on sheet 2 (index 1) in these SAP exports
            # Z-Recon/Invoice usually on sheet 1 (index 0)
            sheet = 1 if "Revenue" in label else 0
            df = pd.read_excel(path, sheet_name=sheet, nrows=5)
            print(f"Columns: {list(df.columns)}")
            print("Top Row Snapshot:")
